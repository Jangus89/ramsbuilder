...
function triggerMatches(haystack, trigger) {
  ...
  if (!trigger.includes(' ') && trigger.length > 4) {
...
export function detectCriticalRiskRules({ taskType, taskDescription, additionalInfo, answers }) {
  ...
  return RULES.filter(rule => rule.triggers.some(trigger => triggerMatches(haystack, trigger)));
}

export function filterCriticalRules(args) {
  return detectCriticalRiskRules(args);
}

export function buildMandatoryHazardsBlock(rules) {
  if (!rules.length) return '';
  const blocks = rules.map(rule => {
    const controls = rule.controls.map(control => `  - ${control}`).join('\n');
    return `- ${rule.title}\n  Hazard: ${rule.hazard}\n  Exact control measures:\n${controls}`;
  });
  return `\nMANDATORY_HAZARDS\nThe following hazards MUST appear in the hazards array with these exact control measures — do not omit or summarise them.\n${blocks.join('\n\n')}\n`;
}
